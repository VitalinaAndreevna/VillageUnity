...
public class GameManagerScript : MonoBehaviour
{
    ...
    private void Start()
    {
        // Отключаем все сокеты, кроме первого 
    ...
    public void OnPartInserted(Collider socket)
    {
        ...
        if (socket.name == sockets[currentStep].name)
        {
            ...
            {
                Debug.Log("inserted " + currentStep + " / 5");
            ...
            {
                Debug.Log("assemled " + currentStep + " / 5");
                AssembledState = 2;
            }
        }
    }
    ...
    public void OnPartRemoved(Collider socket)
    {
        ...
        if (socket.name == sockets[currentStep - 1].name)
        {
            ...
            if (currentStep > 0)
            {
                Debug.Log("removed " + currentStep + " / 5");
            ...
            {
                Debug.Log("disassemled" + currentStep + " / 5");
                AssembledState = 0;
            }
        }
    }
    ...
    public void OnAKInserted()
    {
        ...
        else if (AssembledState == 0)
        {
            Debug.Log("NO");
            IsChallengeRunning = false;
            //DisRecordManager.instance.NewRecordCheck(endTime);
        }
    }
